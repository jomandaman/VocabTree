...
//database.cpp
    ...
    public:
        ...
        tuple<Mat, string, Mat> spatial_verification( Mat& query,  vector<string>& img_path_list,  string& method) {
            ...
            for (const string& img_path : img_path_list) {
                ...
                auto correspondences = fd.detectAndMatch(img, query, method); // Assuming detect_and_match returns a suitable data structure
                
                int inliers;
                Mat optimal_H;
                tie(inliers, optimal_H) = RANSAC_find_optimal_Homography(correspondences, 2000); // Assuming RANSAC_find_optimal_Homography returns a pair
    ...
    int main(int argc, char* argv[]) {
        ...
        string test_path = "./data/DVD-Query-50-1";
        ...
        string img_path = test_path + "/26.jpg";
